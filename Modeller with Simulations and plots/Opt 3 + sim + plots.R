...
# Correlated state innovation covariance matrix
...
random_start <- function() {
  c(
    runif(1, 0.0001, 7.50),   # kappa_x
    runif(1, -0.10, 0.30),    # theta_x
    runif(1, 1e-5, 1.00),     # sigma_x
    
    runif(1, 0.0001, 7.50),   # kappa_y
    runif(1, -0.10, 0.30),    # theta_y
    runif(1, 1e-5, 1.00),     # sigma_y
    
    runif(1, 0.0001, 0.30),   # alpha
    runif(1, -0.10, 0.10),    # eta
    
    runif(1, 1e-7, 0.10),     # sigma_meas_P
    runif(1, 1e-7, 0.10),     # sigma_meas_A
    
    runif(1, -0.999, 0.999),  # rho_xy
...
cat("\nEstimated parameters with rho_xy:\n")
...
cat("\nAll starts sorted by objective value:\n")
print(all_param_real[order(all_param_real$objective), ])
...
legend(
  ...
  cex = 1.25,
  xpd = NA,
  legend = c("Simulated observed", "Optimization 3"),
  col = cols,
  lty = 1,
  lwd = c(1.8, 1.6)
)
...
# Optional:
# save.image("~/Desktop/ekf_workspace_tilføj_a.RData")
# load("~/Desktop/ekf_workspace_tilfoej_a.RData")
