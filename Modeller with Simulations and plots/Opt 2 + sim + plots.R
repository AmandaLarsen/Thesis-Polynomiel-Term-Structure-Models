...
legend(
  ...
  cex = 1.25,
  xpd = NA,
  legend = c("Simulated observed", "Optimization 2"),
  col = cols,
  lty = 1,
  lwd = c(1.8, 1.6)
)
...
check_bounds <- data.frame(
  parameter = names(ekf_out_real$pars),
  estimate = as.numeric(theta_hat),
  lower = lower,
  upper = upper,
  at_lower = abs(theta_hat - lower) < 1e-6,
  at_upper = abs(theta_hat - upper) < 1e-6
)
...
# Optional:
# save.image("~/Desktop/ekf_workspace_udvidet_int_model.RData")
load("~/Desktop/ekf_workspace_udvidet_int_model.RData")
