...
plot_rmse_two_lines <- function(taus, rmse_1, rmse_2, title,
                                label_1 = "Predicted", label_2 = "Filtered",
...
plot_swap_grid(
  ...
  "CIR REAL DATA: observed vs predicted swaps",
...
mtext("REAL DATA: observed vs predicted swaps", outer = TRUE, cex = 1.4)
legend(
  "bottom",
  inset = -0.02,
...
for (j in seq_along(taus)) {
  ...
  y_min <- min(
    Y_real[-c(1,2), j],
    Y_filt_vas_real[-c(1,2), j],
    Y_filt_cir_real[-c(1,2), j],
    na.rm = TRUE
  )
  ...
  lines(dates, Y_real[, j], lwd = 1.8, col = "black")
  lines(
    dates[-c(1,2)],
    Y_filt_vas_real[-c(1,2), j],
    lty = 1,
...
mtext("REAL DATA: predicted, Vasicek vs CIR", outer = TRUE, cex = 1.4)
legend(
  "bottom",
  inset = -0.02,
...
plot_rmse_two_lines(
  ...
  "REAL DATA: Predicted RMSE, Vasicek vs CIR",
  label_1 = "Vasicek", label_2 = "CIR",
  col_1 = "red", col_2 = "blue"
)

###############################################################
# VASICEK SIMULATION STUDY
###############################################################
...
cat("\nVASICEK SIMULATION: residual SD:\n")
...
plot_swap_grid(
  ...
  "VASICEK SIMULATION: observed vs predicted swaps",
...
plot_swap_grid(
  ...
  "CIR SIMULATION: observed vs predicted swaps",
...
###############################################################
#  SIMULATION COMPARISON: VASICEK VS CIR
###############################################################

plot_rmse_two_lines(
  taus, rmse_pred_vas_sim, rmse_pred_cir_sim,
  "SIMULATION:  RMSE, Vasicek vs CIR",
...
state_df <- data.frame(
  Date = dates,
  Vasicek = res_vas_real$filtered$r_filt,
  CIR = res_cir_real$filtered$r_filt
)

ggplot(state_df, aes(x = Date)) +
  
  geom_line(
    aes(y = Vasicek, color = "Vasicek"),
    linewidth = 1
  ) +
  ...
  theme(
    ...
    axis.text.x = element_text(
      size = 16,      # <-- større årstal
      angle = 45,
      hjust = 1
    ),
    
    axis.text.y = element_text(
      size = 15       # <-- større tal på y-aksen
    ),
...
##########################################
# PLots brugt i speciale
##########################################



model_names <- c("Vasicek", "CIR")
...
for (j in seq_along(taus)) {
  ...
  y_min <- min(
    Y_real[idx, j],
    Y_pred_vas_real[idx, j],
    Y_pred_cir_real[idx, j],
    na.rm = TRUE
  )
  ...
  lines(dates[idx], Y_real[idx, j], lwd = 1.8, col = "black")
  lines(dates[idx], Y_pred_vas_real[idx, j], lwd = 1.5, col = cols[2])
  lines(dates[idx], Y_pred_cir_real[idx, j], lwd = 1.5, col = cols[3])
}
...
# Fælles y-akse for alle residual plots
y_min <- min(
  resid_pred_vas_real[idx, ],
  resid_pred_cir_real[idx, ],
  na.rm = TRUE
)
...
rmse_baseline <- tibble(
  Maturity = taus,
  Vasicek = sqrt(colMeans(resid_pred_vas_real[idx, ]^2, na.rm = TRUE)),
  CIR = sqrt(colMeans(resid_pred_cir_real[idx, ]^2, na.rm = TRUE))
) %>%
  pivot_longer(
    cols = c(Vasicek, CIR),
    names_to = "Model",
...
plot(
  ...
  ylab = expression(X[t]),
  main = "Vasicek",
  ylim = range(r_true_vas, r_filt_vas_est, na.rm = TRUE)
)
abline(h = axTicks(2), col = "grey85", lwd = 0.8)
lines(r_filt_vas_est, lwd = 1.8, col = "#ff69b4")
...
plot(
  ...
  ylab = expression(X[t]),
  main = "CIR",
  ylim = range(r_true_cir, r_filt_cir_est, na.rm = TRUE)
)
abline(h = axTicks(2), col = "grey85", lwd = 0.8)
lines(r_filt_cir_est, lwd = 1.8, col = "#1f77b4")
...
idx_sim <- 1:nrow(Y_sim_vas)

cols <- c(
  "black",     # Simulated observed swaps
  "#ff69b4"    # Vasicek fitted swaps
)
...
for (j in seq_along(taus)) {
  ...
  y_min <- min(Y_sim_vas[idx_sim, j], Y_filt_vas_sim[idx_sim, j], na.rm = TRUE)
  y_max <- max(Y_sim_vas[idx_sim, j], Y_filt_vas_sim[idx_sim, j], na.rm = TRUE)
...
legend(
  ...
  cex = 1.25,
  xpd = NA,
  legend = c("Simulated observed", "Vasicek filtered"),
  col = cols,
  lty = 1,
  lwd = c(1.8, 1.6)
)
...
idx_sim <- 1:nrow(Y_sim_cir)

cols <- c(
  "black",     # Simulated observed swaps
  "#1f77b4"    # CIR fitted swaps
)
...
for (j in seq_along(taus)) {
  ...
  y_min <- min(Y_sim_cir[idx_sim, j], Y_filt_cir_sim[idx_sim, j], na.rm = TRUE)
  y_max <- max(Y_sim_cir[idx_sim, j], Y_filt_cir_sim[idx_sim, j], na.rm = TRUE)
...
legend(
  ...
  cex = 1.25,
  xpd = NA,
  legend = c("Simulated observed", "CIR filtered"),
  col = cols,
  lty = 1,
  lwd = c(1.8, 1.6)
)
