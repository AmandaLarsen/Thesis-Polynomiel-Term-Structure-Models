...
ukf_sigma_points <- function(
  ...
  if (!is.finite(cfac) || cfac <= 0) {
    stop("UKF error: L + lambda must be positive")
  }
...
ukf_filter <- function(theta_vec, Z_obs) {
  ...
  P_pred <- array(NA_real_, c(L, L, n))
  P_filt <- array(NA_real_, c(L, L, n))
  ...
  for (t in 1:n) {
    ...
    if (any(!is.finite(Zsig))) {
      stop("Non-finite measurement sigma points")
    }
    ...
    z_t_filt <- measurement_function(x_t_filt, pars)
    
    if (any(!is.finite(z_t_filt))) {
      stop("Non-finite filtered measurement")
    }
...
ukf_out_real <- ukf_filter(theta_hat, Z_obs)

cat("\nEstimated parameters with rho_xy and a:\n")
print(ukf_out_real$pars)

cat("\nBest start:", best_idx, "out of", length(fit_list), "successful starts\n")
cat("Best objective value:", best_fit$value, "\n")

param_real <- data.frame(
  parameter = names(ukf_out_real$pars),
  estimate  = as.numeric(ukf_out_real$pars)
)

print(param_real)

cat("\nEstimated model correlation rho_xy:\n")
print(ukf_out_real$pars["rho_xy"])

cat("\nEstimated a:\n")
print(ukf_out_real$pars["a"])

cat("\nEmpirical correlation between filtered states X and Y:\n")
print(cor(ukf_out_real$x_filt[, 1], ukf_out_real$x_filt[, 2], use = "complete.obs"))

cat("\nEmpirical correlation between predicted states X and Y:\n")
print(cor(ukf_out_real$x_pred[, 1], ukf_out_real$x_pred[, 2], use = "complete.obs"))
...
plot_df_real <- data.frame(
  Date = dates,
  X_filtered = ukf_out_real$x_filt[, 1],
  Y_filtered = ukf_out_real$x_filt[, 2]
)
...
swap_pred_real <- t(sapply(seq_len(nrow(ukf_out_real$x_pred)), function(i) {
  model_swap_rates(
    state = ukf_out_real$x_pred[i, ],
    pars  = ukf_out_real$pars
  )
}))

swap_filt_real <- t(sapply(seq_len(nrow(ukf_out_real$x_filt)), function(i) {
  model_swap_rates(
...
run_simulation_study_current_model <- function(
  ...
  ukf_true <- ukf_filter(true_pars, sim$Z_obs)
  
  cat("RMSE X, true-param filter:", rmse(sim$X_true, ukf_true$x_filt[, 1]), "\n")
  cat("RMSE Y, true-param filter:", rmse(sim$Y_true, ukf_true$x_filt[, 2]), "\n")
  
  cat("\nObservation RMSE, true parameters, Z_pred vs Z_true:\n")
  print(data.frame(
    series = colnames(Z_obs),
    rmse = sqrt(colMeans((sim$Z_true - ukf_true$Z_pred)^2))
  ))
  ...
  true_vs_est <- data.frame(
    ...
    estimated = as.numeric(fit_sim$ukf$pars)
  )
  ...
  cat("RMSE X:", rmse(sim$X_true, fit_sim$ukf$x_filt[, 1]), "\n")
  cat("RMSE Y:", rmse(sim$Y_true, fit_sim$ukf$x_filt[, 2]), "\n")
  
  cat("\nObservation RMSE, estimated parameters,Z_pred vs Z_true:\n")
  print(data.frame(
    series = colnames(Z_obs),
    rmse = sqrt(colMeans((sim$Z_true - fit_sim$ukf$Z_pred)^2))
  ))
...
plot_df_sim <- data.frame(
  ...
  X_pred_truepar = res_simstudy$ukf_true$x_pred[, 1],
  X_filt_truepar = res_simstudy$ukf_true$x_filt[, 1],
  X_pred_estpar = res_simstudy$fit_sim$ukf$x_pred[, 1],
  X_filt_estpar = res_simstudy$fit_sim$ukf$x_filt[, 1],
  Y_true = res_simstudy$sim$Y_true,
  Y_pred_truepar = res_simstudy$ukf_true$x_pred[, 2],
  Y_filt_truepar = res_simstudy$ukf_true$x_filt[, 2],
  Y_pred_estpar = res_simstudy$fit_sim$ukf$x_pred[, 2],
  Y_filt_estpar = res_simstudy$fit_sim$ukf$x_filt[, 2]
)
...
swap_pred_sim <- t(sapply(seq_len(nrow(res_simstudy$fit_sim$ukf$x_pred)), function(i) {
  model_swap_rates(
    state = res_simstudy$fit_sim$ukf$x_pred[i, ],
    pars  = res_simstudy$fit_sim$ukf$pars
  )
}))

swap_filt_sim <- t(sapply(seq_len(nrow(res_simstudy$fit_sim$ukf$x_filt)), function(i) {
  model_swap_rates(
    state = res_simstudy$fit_sim$ukf$x_filt[i, ],
    pars  = res_simstudy$fit_sim$ukf$pars
  )
}))
...
###############################################################
# INITIAL MODEL: SIMULATION TRUE VS PREDICTED SWAPS
###############################################################
...
legend(
  ...
  cex = 1.25,
  xpd = NA,
  legend = c("Simulated observed", "Optimization 5 UKF"),
  col = cols,
  lty = 1,
  lwd = c(1.8, 1.6)
)
...
# save.image(file.path(getwd(), "ukf_workspace_tilfoej_a_med_andet_udvidet_int.RData"))
# load("~/Desktop/ukf_workspace_tilfoej_a_med_andet_udvidet_int.RData")
